...
func new_reading_question():
	...
	#place drop_zones
	place_drop_zones($lbl_word1)
	place_drop_zones($lbl_word2)
	place_drop_zones($lbl_word3)
	place_drop_zones($lbl_word4)
	place_drop_zones($lbl_word5)
	place_drop_zones($lbl_word6)
...
func place_drop_zones(lbl : Label):
		if lbl.text.contains("ADJECTIVE"):
			$drop_zone_adjective.position = lbl.position + (lbl.size / 2)
		if lbl.text.contains("VERB"):
			$drop_zone_verb.position = lbl.position + (lbl.size / 2)
		if lbl.text.contains("NOUN"):
			$drop_zone_noun.position = lbl.position + (lbl.size / 2)
			
func lbl_invis(lbl : Label):
	if $draggable_icon0.position == lbl.position + (lbl.size/2) || $draggable_icon1.position == lbl.position + (lbl.size/2) || $draggable_icon2.position == lbl.position + (lbl.size/2):
		lbl.visible = false
	else:
		lbl.visible = true
...
func filled(zone : StaticBody2D):
	if zone.position != $draggable_icon0.position && zone.position != $draggable_icon1.position && zone.position != $draggable_icon2.position:
		return false
	else:
		return true

func _on_btn_check_pressed() -> void:
	if !filled($drop_zone_adjective) || !filled($drop_zone_verb) || !filled($drop_zone_noun):
		$score.text = "Fill all the blanks!"
	elif $drop_zone_adjective.position == $draggable_icon0.position && $draggable_icon0.get_meta("type") != "ADJECTIVE":
		new_reading_question()
